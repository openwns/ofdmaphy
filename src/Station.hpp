...
namespace ofdmaphy {
		...
		public rise::Station
	{
		...
		// @brief typedef and functor for CarrierSensing::onRSSChange calls
		typedef wns::service::phy::ofdma::CarrierSensing CarrierSensing;
		struct OnCS
		{
			OnCS(const wns::Power _rss) :
				rss(_rss)
			{
			}

			void operator()(CarrierSensing* cs)
			{
				cs->onRSSChange(this->rss);
			}
	...
	public:
 	...
 	protected:
		...
		/** @brief retrieve info about the used power per subband for a
		 * certain user, currently not implemented */
		virtual wns::Power
		getSumPower() const;
