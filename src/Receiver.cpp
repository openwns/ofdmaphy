	...
	samplingTime(0.0)
{
	...
	if (!config.isNone("FTFadingStrategy")) {
		...
		if (!ftfadingView.isNone("ftfadingName")) {
			...
			//std::cout << "FTFadingAspect.logger: "<<logger.getModuleName()<<"."<<logger.getLoggerName()<<std::endl;
			assure(logger.getModuleName().compare("unspecified")!=0,"logger problem");
			...
			FTFadingCreator = rise::scenario::ftfading::FTFading::FTFadingFactory::creator(ftfadingName);
			...
			if (samplingTime > 0.0) {
				...
				active=true;
			} else { // effectively fading is off
...
wns::Ratio FTFadingAspect::getFTFading(wns::node::Interface* source, int _subCarrier)
{
	...
	if (myFTFading==NULL) { // create on demand
		myFTFading = ftfading; // copy the single fading object for all sources
	...
	measurementUpdateOffset(0.0)
{
	...
	if (logger.getModuleName().compare("unspecified")==0)
		logger=wns::logger::Logger(config.getView("logger")); // workaround.
	if (doMeasurementUpdates) { // can also be done if ftfading == NULL
		if (!config.isNone("measurementUpdateInterval")) {
			measurementUpdateInterval = config.get<simTimeType>("measurementUpdateInterval");
			if (measurementUpdateInterval==0.0) { doMeasurementUpdates=false; }
		...
		// in regular intervals: ofdmaStation->measurementUpdate(source,ofdmaMeasurementPtr);
		//startRegularMeasurementUpdates(); // done in Receiver constructor
	} else {
		measurementUpdateInterval=0.0;
	}
}
...
{
	if (!doMeasurementUpdates) return;
	assure(source!=NULL,"source==NULL");
	int numberOfSubChannels=getCurrentNumberOfSubCarriers();
	//MESSAGE_SINGLE(NORMAL, logger, "registerSource("<<source->getName()<<") ?");
	PerSourceMap::iterator perSourceMapFound = perSourceMap.find(source);
	//assure(perSourceMapFound!= perSourceMap.end(),"cannot find source in perSourceMap");
	if (perSourceMapFound == perSourceMap.end()) { // not found
...
{
	assure(source!=NULL,"source==NULL");
	//MESSAGE_SINGLE(NORMAL, logger, "saveMeasuredFlatPathloss("<<source->getName()<<","<<pathloss<<")");
	//PerSourceContainer myPerSourceContainer = perSourceMap[source];
	PerSourceMap::iterator perSourceMapFound = perSourceMap.find(source);
	//assure(perSourceMapFound!= perSourceMap.end(),"cannot find source "<<source->getName()<<" in perSourceMap");
	...
	//doMeasurementUpdates(config.get<bool>("doMeasurementUpdates")),
{
	...
	if (measurementUpdatesAreOn()) { // can also be done if ftfading == NULL
		...
		if ((getMeasurementUpdateInterval()==0.0) && (ftfading)) {
			...
			assure(getMeasurementUpdateInterval() == ftfading->getSamplingTime() ,"measurementUpdateInterval="<<getMeasurementUpdateInterval()<<" != ftfading::samplingTime="<<ftfading->getSamplingTime());
		} else {
		}
		assure(getMeasurementUpdateInterval()>0.0,"doMeasurementUpdates=True but measurementUpdateInterval=0");
...
wns::Ratio Receiver::getQuasiStaticPathLoss(const rise::TransmissionObjectPtr& t, wns::service::phy::ofdma::PatternPtr pattern)
{
	...
	if (pattern == wns::service::phy::ofdma::PatternPtr()) { // no pattern = no beamforming
		// static antenna
...
void Receiver::notify(rise::TransmissionObjectPtr t)
{
	if (transmissionForMe(t)) {
		...
		//registerSource(sourceNode);
		wns::node::Interface* sourceNode = registerSource(t); // for OFDMA measurements
		if (t->getIsStart()) { // start of transmission
			...
			if (find(activeTransmissions.begin(), activeTransmissions.end(), t) != activeTransmissions.end()) {
				...
				if (doMeasurementUpdates) { // the periodic "big" ones
				...
				//MESSAGE_SINGLE(NORMAL, logger, "ofdmaphy::Receiver::receiveData(): preparing PowerMeasurementPtr");
...
Receiver::doMeasurementsNow()
{
	...
	for (PerSourceMap::iterator perSourceIterator = perSourceMap.begin(); perSourceIterator != perSourceMap.end(); ++perSourceIterator)
	{
		...
		assure((int)perSourceContainer.interferenceVector.size()==numberOfSubChannels,"wrong interferenceVector.size="<<perSourceContainer.interferenceVector.size());
